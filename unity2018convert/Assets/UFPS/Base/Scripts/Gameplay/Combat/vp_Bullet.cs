...
public class vp_Bullet : MonoBehaviour
{
	...
	protected virtual bool TryHit()
	{
		...
		else if((m_Source != null) && (m_Source.gameObject.layer == vp_Layer.Enemy))
            LayerMask = vp_Layer.Mask.enemyBullet;
        else
		...
		if ((m_Source != null) && (m_Source.gameObject.layer == vp_Layer.Enemy)) {
			if (m_Hit.collider.gameObject.layer == vp_Layer.Enemy) {
				TryDestroy ();
				return true;
			}
		}
