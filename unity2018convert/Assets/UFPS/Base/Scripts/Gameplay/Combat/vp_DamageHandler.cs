...
public class vp_DamageHandler : MonoBehaviour
{
    ...
    public GameObject[] acitveObjects = null;       // gameobjects to spawn when object dies.
    public GameObject[] deacitveObjects = null;       // gameobjects to spawn when object dies.
	...
	public virtual void Damage(vp_DamageInfo damageInfo)
	{
		...
		if (CurrentHealth <= 0.0f)
		{
            if (acitveObjects != null)
            foreach (GameObject child in acitveObjects)
            {
                child.SetActive(true);
            }
            if(deacitveObjects!=null)
            foreach (GameObject child in deacitveObjects)
            {
