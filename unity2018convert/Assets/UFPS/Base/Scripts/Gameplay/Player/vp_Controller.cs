		 ...
		 && (m_GroundHitTransform != m_LastGroundHitTransform))
		{
			...
			if (m_LastGroundHitTransform == null)
			{
				...
				if ((m_FallStartHeight > Transform.position.y) && m_Grounded)
				{
                    ...
                    Player.Health.Set(Player.Health.Get() - 0.5f);
                    if (Player.Health.Get() <= 0.0f)
                    {
                        vp_LocalPlayer.Die();
                        //Player.FallImpact.Send(m_FallImpact);
                        //GameObject.Find("AdvancedPlayerMobileNGUI").gameObject.SetActive(false);
                        GameObject.Find("LevelRenderSetting").GetComponent<LevelSetting>().OnFailure();
                    }
                    //Player.Health.Set(0.5f);
                    //m_FallImpact = 0.0f;
                    //Debug.Log("m_FallImpact: " + m_FallImpact.ToString());
                }
			}
			m_FallStartHeight = NOFALL;
