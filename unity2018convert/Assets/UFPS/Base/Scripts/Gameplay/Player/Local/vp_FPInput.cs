...
// when developing for GearVR. please report if it causes any trouble
		...
		if (m_MouseLookSmoothMove.magnitude > 0.6f) {
			m_LastMouseLookFrame = Time.frameCount;
		}
		// --- mouse smoothing ---
		...
		m_CurrentMouseLook.x *= (MouseLookSensitivity.x*800f/(float)Screen.width + mouseAcceleration);
		m_CurrentMouseLook.y *= (MouseLookSensitivity.y*480f/(float)Screen.height + mouseAcceleration);

		m_CurrentMouseLook.y = (MouseLookInvert ? m_CurrentMouseLook.y : -m_CurrentMouseLook.y);

		return m_CurrentMouseLook*Screen.dpi;
